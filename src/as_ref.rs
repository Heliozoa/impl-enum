...
fn make_arms(input_enum: &ItemEnum) -> syn::Result<Vec<TokenStream2>> {
    ...
    for variant in &input_enum.variants {
        ...
        let match_arm = if let Some(first_field_ident) = &first_field.ident {
            quote::quote! {
                Self::#variant_ident { #first_field_ident: __first, .. } => __first.as_ref()
            }
        } else {
            quote::quote! {
                Self::#variant_ident ( __first, .. ) => __first.as_ref()
            }
        };
